...
namespace ContactBookAPI.BL
{
    ...
    public class BLConverter
    {
        ...
        public DataTable ToDataTable<T>(List<T> obj) where T : class
        {
            ...
            // Get the properties of the type T (the object model)
            ...
            // Iterate through each object in the list and add it as a row
            foreach (var item in obj)
            {
                DataRow row = dataTable.NewRow();
                foreach (var property in properties)
                {
                    row[property.Name] = property.GetValue(item) ?? DBNull.Value; // Handle null values
                }
