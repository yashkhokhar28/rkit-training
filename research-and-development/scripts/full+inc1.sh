...
# This script creates a MySQL test database, populates it with data,
# performs full and incremental backups using Percona XtraBackup,
# prepares the backups, restores the data, and validates the restoration.
# It is designed to be portable and includes error handling and configuration options.
...
INCREMENTAL_DIR="/tmp/xtrabackup_inc" # Directory for incremental backup
...
check_directory_writable "$(dirname "$INCREMENTAL_DIR")"
check_mysql_connection
...
# Insert an additional 50 rows
log "Inserting 50 more rows into both tables..."
for i in {101..150}; do
    $MYSQL -e "
    USE test;
    INSERT INTO orders1 VALUES ($i, 'Item1_$i');
...
log "Performing incremental backup to ${INCREMENTAL_DIR}..."
rm -rf "$INCREMENTAL_DIR" # Clear any existing incremental backup
xtrabackup --backup --target-dir="$INCREMENTAL_DIR" --incremental-basedir="$BACKUP_DIR" --user="$MYSQL_USER" ${MYSQL_PASSWORD:+--password="$MYSQL_PASSWORD"}

# Prepare the full backup
log "Preparing full backup (apply-log-only)..."
xtrabackup --prepare --apply-log-only --target-dir="$BACKUP_DIR"

# Merge the incremental backup
log "Merging incremental backup..."
xtrabackup --prepare --target-dir="$BACKUP_DIR" --incremental-dir="$INCREMENTAL_DIR"

# Final preparation
log "Final prepare (make ready to restore)..."
