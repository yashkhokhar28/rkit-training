$( document ).ready( () => {
    ...
    var DateBoxInstance = $( "#DateTime" )
        .dxDateBox( {
            ...
            onContentReady: ( e ) => console.log( "DateBox is ready!" ),

            onCopy: ( e ) => alert( "Copied: " + e.component.option( "text" ) ),

            onCut: ( e ) => alert( "Text cut from input!" ),

            onDisposing: () => console.log( "DateBox is being removed!" ),

            onEnterKey: ( e ) =>
                console.log( "Enter key pressed! Selected date:", e.value ),

            onInitialized: ( e ) => console.log( "DateBox initialized!" ),

            onInput: ( e ) =>
                console.log( "User is typing:", e.component.option( "text" ) ),

            onKeyDown: ( e ) => console.log( "Key Down:", e.event.key ),

            onKeyPress: ( e ) => console.log( "Key Press:", e.event.key ),

            onKeyUp: ( e ) => console.log( "Key Up:", e.event.key ),

            onOpened: () => console.log( "DateBox dropdown opened!" ),

            onOptionChanged: ( e ) =>
                console.log( "Option changed:", e.name, "New value:", e.value ),

            onPaste: ( e ) => alert( "Text pasted into the input!" ),
        } )
        .dxDateBox( "instance" );
    ...
    // Get calendar drop-down content
    var content = DateBoxInstance.content();
    console.log( content );

    // Get DateBox container element
    console.log( DateBoxInstance.element() );

    // Get input field element
    console.log( DateBoxInstance.field() );

    // Get "Clear" button instance
    var clearButton = DateBoxInstance.getButton( "clear" );
    console.log( clearButton );

    //// Attach event handlers dynamically
    //DateBoxInstance.on( {
    //    valueChanged: ( e ) => console.log( "Value changed:", e.value ),
    //    opened: () => console.log( "Calendar opened!" ),
    //} );

    // Dynamically update an option
    DateBoxInstance.option( "placeholder", "Pick a date" );

    // Register a custom key handler (Enter key)
    DateBoxInstance.registerKeyHandler( "enter", () => {
